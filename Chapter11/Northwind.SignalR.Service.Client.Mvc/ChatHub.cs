...
public class ChatHub : Hub
{
    ...
    public async Task Register(UserModel newUser)
    {
        ...
        if (Users.TryGetValue(newUser.Name, out UserModel? value))
        {
            user = value;
    ...
    public async Task SendMessage(MessageModel message)
    {
        ...
        {
            if (Users.TryGetValue(userOrGroup, out UserModel? value))
            {
                // If the item is in Users then send the message to that user
                // by looking up their connection ID in the dictionary.
                message.To = $"User: {value.Name}";
                proxy = Clients.Client(value.ConnectionId);
            }
